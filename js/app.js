(() => {
    ...
    function buildBlogItem(item) {
        let blogItemTeamplate = ``;
        blogItemTeamplate += `<article data-id="${item.id}" class="blog__item item-blog">`;
        item.image ? blogItemTeamplate += `<a href="${item.url}" class="item-blog__image-ibg">\n\t\t<img src="${item.image}" alt="image">\n\t</a>` : null;
        blogItemTeamplate += `<div class="item-blog__date">${item.date}</div>`;
        blogItemTeamplate += `<h4 class="item-blog__title">\n            <a href="${item.url}" class="item-blog__link-title">${item.title}</a>\n        </h4>`;
        item.text ? blogItemTeamplate += `<div class="item-blog__text text">\n\t\t\t\t${item.text}\n\t\t\t</div>` : null;
        if (item.tags) {
            blogItemTeamplate += `<div class="item-blog__tags">`;
            for (const tag in item.tags) blogItemTeamplate += `<a href="${item.tags[tag]}" class="item-blog__tag">${tag}</a>`;
            blogItemTeamplate += `</div>`;
        }
        blogItemTeamplate += `</article>`;
        blogItems.insertAdjacentHTML("beforeend", blogItemTeamplate);
    }
